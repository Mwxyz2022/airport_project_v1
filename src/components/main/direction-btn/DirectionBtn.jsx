...
const DirectionBtn = ({ onSearchTextHandler }) => {
  const searchDate = moment().format('DD-MM-YYYY')

  const searchString = `?date=${searchDate}`

  return (
    <section className="direction">
      <Link
        to={{ pathname: '/departures', search: searchString }}
        className="btn btn__departures"
        onClick={onSearchTextHandler}
      >
      ...
      <Link
        to={{ pathname: '/arrivals', search: searchString }}
        className="btn btn__arrivals"
        onClick={onSearchTextHandler}
      >
        <div className="btn__container">
